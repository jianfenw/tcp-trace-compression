...
class TcpEndpoint():
    ...
    def __init__(self, annotated_packet, use_source):
        ...
        # use_source = True -- endpoint_a
        # use_source = False -- endpoint_b
        # mss: 
        if use_source:
            self.ip = ip.src
    ...
    def add_packet(self, annotated_packet, process_packet=True):
        ...
        for packet in wire_packets:
            ...
            # packet.seq_end != packet.seq  <==> packet.data_len != 0
            if packet.seq_end != packet.seq and process_packet:
                """
                    Since we don't have any ack information, we have to look
                    at the seq_end ...
                    i.e. self.seq_next = packet.seq_end
                """
